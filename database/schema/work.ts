...
export const modules = appSchema.table(
  ...
  (table) => [
    ...
    check(
      "modules_description_check",
      sql.raw("length(description) <= 20000")
    ),
    check(
      "modules_kind_check",
      sql.raw("kind IN ('NORMAL', 'UNCLASSIFIED')")
    ),
    check(
      "modules_status_check",
      sql.raw("status IN ('ACTIVE', 'ARCHIVED')")
    ),
...
export const features = appSchema.table(
  ...
  (table) => [
    ...
    check(
      "features_tags_check",
      sql.raw(
        "cardinality(tags) <= 50 AND array_position(tags, NULL) IS NULL"
      )
    ),
    check(
...
export const tasks = appSchema.table(
  ...
  (table) => [
    ...
    check(
      "tasks_description_check",
      sql.raw("length(description) <= 50000")
    ),
    check(
...
export const taskStatusHistory = appSchema.table(
  ...
  (table) => [
    ...
    check(
      ...
      sql.raw(
        "from_work_status IS NULL OR from_work_status <> to_work_status"
      )
    ),
    check(
