...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
  throw 'Docker is required to run the search PoC.'
}
...
$containerName = 'inpulse-poc-search-' + [System.Guid]::NewGuid().ToString('N')
$created = $false
$succeeded = $false
...
try {
  ...
  $env:POC_POSTGRES_IMAGE = $Image
  $env:POC_POSTGRES_IMAGE_DIGEST = $imageDigest
  ...
  try {
    ...
    & pnpm db:poc:search
    if ($LASTEXITCODE -ne 0) {
      throw "search PoC failed with exit code $LASTEXITCODE"
    }
  } finally {
    Pop-Location
  }
...
if (-not $succeeded) {
  throw 'Search PoC local run did not complete successfully.'
}
