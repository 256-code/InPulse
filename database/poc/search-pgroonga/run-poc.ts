...
async function checkRuntimePermissions(
  ...
  try {
    ...
    if (message.includes("permission denied") || message.includes("42501") || message.includes("pgroonga_regexp_text")) {
...
async function runStrategy(
  ...
  try {
    ...
    const diagnosticSql = strategy.queryStyle === "regex"
      ? adminSql
      : runtimeSql;
    const baseIndex = await createIndexAndMeasure(adminSql, BASE_TABLE, strategy);
    const baseSuite = await runSuite(diagnosticSql, BASE_TABLE, strategy, projectIds, seed);
    const basePlans = await runPlanChecks(diagnosticSql, BASE_TABLE, strategy, projectIds);
    const probeIndex = await createIndexAndMeasure(adminSql, PROBE_TABLE, strategy);
    const requirementProbe = await runRequirementProbe(diagnosticSql, PROBE_TABLE, strategy);
    const scaleIndex = await createIndexAndMeasure(adminSql, SCALE_TABLE, strategy);
    const scaleSuite = await runSuite(diagnosticSql, SCALE_TABLE, strategy, projectIds, seed);
    const scalePlans = await runPlanChecks(diagnosticSql, SCALE_TABLE, strategy, projectIds);
    let updateMs = -1;
    let reindexMs = -1;
...
async function runPoc(): Promise<void> {
  ...
  try {
    ...
    const failedStrategies = outcomes.filter((outcome) => outcome.error !== null);
