...
interface MyTasksQueryRequest {
  ...
  readonly sort?: "id";                   // 见 Q-10（已由 ADR-036 替代：排序固定，不提供 sort）
}
```
...
| Q-10 | R-3 的排序键与游标键？现有索引可命中 `ORDER BY t.id DESC`，不能命中按 `updated_at` 排序 | §3.3.2、端口提案 §1.4 | `id DESC`；若产品要求按更新时间排序，需先补索引 | 固定 `id DESC`，不提供 `sort`；`limit` 默认 20、上限 100；游标沿用 C-006。2026-09-18 由 [ADR-036](adr/ADR-036.md) 替代：固定「状态分组 + 紧急桶 + 优先级 + 截止时间 + 任务 ID」，`MY_TASKS` 游标载荷扩展为多列 keyset（待人工批准） |
