...
>
> 2026-09-23 注：项目归档已由 [ADR-043](adr/ADR-043.md)、模块归档与恢复已由 [ADR-044](adr/ADR-044.md) 整体下线（模块只有 `ACTIVE`，`archiveModule` / `restoreModule` 两条路由与 `ModuleArchiveRequest` 已从契约删除，模块列表不再有归档入口）。下文的路由表、状态语义与「归档模块只读 / 恢复」描述均为 2026-09-09 交审时的事实。
...
### 主线同步（2026-09-09，Push 4）

前一提交 `54cfbe1` 的 [CI run 34328536541](https://github.com/256-code/InPulse/actions/runs/34328536541) 已全部通过。随后 main 合入 `3a8158f`，与本分支在应用装配、开发日志冲突；本次合并保留双方装配与全部日志，并适配主线新增的 @Operation 强制绑定要求。模块真库/HTTP 测试接入全局异常过滤器和响应校验器后 9/9 通过；局部 API/HTTP 测试 18/18、契约/绑定 16/16、契约漂移及改动 TS 文件格式检查通过。生成物与业务规则未改变。

按用户最新要求，本次直接推送，不等待或跟踪 CI；自动触发配置不变，本次 CI 状态未验证。PR 仍待非作者评审，不自行合并。
