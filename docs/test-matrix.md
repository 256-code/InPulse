...
| ID | 层级 | 场景 | 通过标准 | 状态 |
|---|---|---|---|---|
| F08-READ-API-001 | HTTP + PostgreSQL | 管理员读取 SYSTEM 链并留痕 | 管理员返回 `AuditLogPage`（SYSTEM 链、64 位十六进制 `prevHash`/`recordHash`、ISO 时间）；开启一次新查看的请求后在 SYSTEM 链恰有一条 `AUDIT_LOG_READ`，`targetId=SYSTEM`，payload 含 `returnedCount`/`hasMore` 与 filters（未筛选操作人时 `actorIds` 为 `null`），不含审计正文 | 本地通过（`apps/api/test/audit-logs.integration.test.ts` 8/8，2026-09-23 更新为「查看」粒度口径与多操作人筛选） |
