...
| `removeProjectMember` · `POST /api/v1/projects/{projectId}/members/{userId}/remove` | 401 | 任意活跃成员（ADR-039；目标为本项目 LEADER 时 409，须先由系统管理员转移/撤销） | 404 | 404 | 401 | 允许（目标为本项目 LEADER 时同样 409） | 目标必须为 ACTIVE 成员；CSRF 与 `Idempotency-Key` 必填；可提交真实任务改派（改派只替换该成员、同一任务的其他负责人不变，[ADR-040](adr/ADR-040.md)），未改派任务保留原负责人但成员立即失去处理权限与角色（REMOVED 行 role 回落 MEMBER）；任务改派、成员移除、审计与活动同一事务；`created_by` 不变；重放前重新验证当前认证、项目可写、成员资源与操作者角色 |
