...
项目创建者不是独立身份。创建时必须作为初始成员且表单不可取消，创建者成员行以 `role='LEADER'` 落库（ADR-033）；创建完成后可由系统管理员转移/撤销其组长角色，或按普通成员规则移除（移除 LEADER 成员行前必须先转移/撤销组长）。`projects.created_by` 永久保留用于溯源，不直接授予权限；项目内权限来自成员行 `role`（`MEMBER | LEADER`，ADR-033 定义、[ADR-039](adr/ADR-039.md) 收敛），角色只在本项目内生效，成员被移除即失效、重新加入从 MEMBER 开始。**ADR-039 起 `LEADER` 只是身份标识，不再附带任何额外管理权**：项目内管理操作只区分「本项目活跃成员」与「非成员」，不再有 403 角色分支；任命/撤销组长只能由系统管理员执行。系统管理员创建者仍保留与成员记录无关的全局权限。详见 [ADR-012](adr/ADR-012.md)（已被 [ADR-033](adr/ADR-033.md) 扩展、[ADR-039](adr/ADR-039.md) 修订）。
...
| archiveModule | 完整认证的系统管理员、本项目任意活跃成员（ADR-039） | 非成员（含已移除）404；原因、If-Match、项目未归档且模块 ACTIVE（ADR-035）；ADR-034 起模块下仍存在未收尾任务（`lifecycle_status = 'ACTIVE'` 且 `work_status = 'TODO'`）时 409 `MODULE_ARCHIVE_TASKS_OPEN`，已完成、已取消或已归档的任务不算阻塞，功能无需归档 |
...
| 变更项目状态（`changeProjectStatus` · `PATCH /api/v1/projects/{projectId}/status`） | 401 | 允许（本项目任意活跃成员，ADR-039） | 404 | 404 | 401 | 允许 | ADR-035 / ADR-039：本项目任意活跃成员或系统管理员可把项目在未开始 / 进行中 / 维护中之间切换，非成员或跨项目 404；归档只能走归档流程，请求体只收三个非归档目标态（ARCHIVED 422 `PROJECT_VALIDATION_FAILED`）；项目已归档 409 `PROJECT_ARCHIVED`，版本不符 409 `PROJECT_VERSION_CONFLICT`，目标与当前状态相同 409 `PROJECT_STATE_CONFLICT`；未开始与维护中禁止直接互改 409 `PROJECT_STATUS_LEVEL_SKIP`，项目内出现过已完成任务后不可回退未开始 409 `PROJECT_STATUS_NOT_STARTED_LOCKED`；CSRF、`Idempotency-Key` 与 `If-Match` 必填；只有「未开始 → 进行中」通知全体活跃成员，维护中不通知；审计 `project.status.change`、活动与搜索投影在同一事务提交；重放前重新验证当前成员关系与项目可读权限 |
...
| 归档任务与恢复任务（`archiveTask`/`restoreTask` · `POST /api/v1/projects/{projectId}/modules/{moduleId}/features/{featureId}/tasks/{taskId}/archive`，模块级任务为去掉 `features/{featureId}` 的同名路径） | 401 | 允许（本项目任意活跃成员，ADR-039） | 404 | 404 | 401 | 允许（完整管理员 Session） | ADR-039：原因、`If-Match`、CSRF 与 `Idempotency-Key` 必填；只切换任务 `lifecycle_status`，工作状态、完成快照与 `task_status_history` 不可变；项目必须未归档（ADR-035）；模块或功能已归档时仍允许归档其任务（收尾），恢复要求模块与功能父级链全部 ACTIVE；审计 `task.archive` 与 `task.unarchive`、活动与搜索投影同一事务提交 |
| 项目归档申请（`requestProjectArchive` · `POST /api/v1/projects/{projectId}/archive-requests`） | 401 | 允许（本项目任意活跃成员，ADR-039） | 404 | 404 | 401 | 允许（完整管理员 Session） | ADR-034 / ADR-035 / ADR-039：项目必须未归档且项目下任务均已收尾（不存在 `lifecycle_status = 'ACTIVE'` 且 `work_status = 'TODO'` 的任务），否则 409 `PROJECT_ARCHIVE_TASKS_OPEN`；同一项目同时只允许一条待审申请，重复申请 409；CSRF 与 `Idempotency-Key` 必填，不要求 `If-Match`；申请不改变项目状态，审计 `project.archive.request`、活动与通知全部系统管理员的站内通知在同一事务提交；重放前重新验证当前成员关系与项目可读权限 |
...
| 任务看板（`getProjectTaskBoard` · `GET /api/v1/projects/{projectId}/task-board`） | 401 | 允许 | 404 | 404 | 401 | 允许 | 只读聚合：SQL 前强制服务端 `AuthorizedProjectScope`，非成员与项目不存在统一 404；看板集合为项目内 `lifecycle_status = 'ACTIVE'` 任务并排除任务组历史来源分支，已取消任务保留为历史标记；完成率 = 已完成 ÷（已完成 + 未完成），已取消不计入分母；逾期、今日到期、本周完成与卡片 `dueState` 全部由 SQL 按 Asia/Shanghai 计算；单项目任务上限 1000 条，超出时 `truncated = true` 且顶部与泳道统计仍为全量口径；泳道头像最多 24 个 |
| 我的任务（`listMyTasks` · `GET /api/v1/me/tasks`） | 401 | 仅本人 | 仅本人 | 仅本人 | 401 | 仅本人 | 负责人固定为当前用户，拒绝客户端提交 `assigneeId`/`userId`/`projectIds`；SQL 前强制 AuthorizedProjectScope；`limit` 1～100、默认 20；游标为服务端 HMAC 签名、绑定当前用户与筛选条件，TTL 15 分钟；`hasPublishedRecord` 与分页在同一条 SQL 内先过滤后分页；V1 筛选为 `projectId`/`scopeType`/`workStatus`/`hasPublishedRecord`/`priority`/`includeCanceled`/`todayTodo`（`priority` 与 `workStatus` 正交；`includeCanceled` 缺省 false，与 `workStatus` 组合表达 `TODO ∪ CANCELED`；`todayTodo` 只表达「未完成且命中 逾期 ∪ 遗留来源（`leftover_task_links` 存在链接）∪ 标记紧急（`priority = 'URGENT'`）∪ 距截止 7 个日历日内 之一」，与服务端统计卡同口径）；条目含 `priority`/`dueAt`/`completedAt`/`creatorId`/`githubLinkCount`/`publishedRecordCount`（与 `hasPublishedRecord` 同源同口径，裁决修订 D-1）/`groupId`/`hasLeftoverSource`（按 `leftover_task_links` 存在链接行判定，与来源记录当前状态无关，裁决修订 D-2）（列表不返回 `description`）；响应附 `stats`（`todayTodo`/`todayTodoBreakdown`（`overdue`/`leftover`/`urgent`/`dueWithinDays`，四项各自独立计数、可以重叠，其并集即 `todayTodo`）/`myOpen`/`completed`/`created`；前三项按负责人维度、`created` 按创建人维度（2026-09-20 口径），日界按 Asia/Shanghai 由服务端计算）、`leftoverCount` 与 `leftoverSample`（最新 ACTIVE 遗留，summary 前 200 字符、超长追加 “…”），统计基准集合不受分页与游标影响 |
...
| 聚合组列表（`listTaskGroups` · `GET /api/v1/task-groups`） | 401 | 仅本人活跃项目 | 不返回本项目（可读其他活跃成员项目） | 不返回本项目（可读其他活跃成员项目） | 401 | 按服务端 Scope | 跨项目按服务端 `AuthorizedProjectScope` 汇总当前用户作为活跃分支任务负责人的聚合组与当前生效（ACTIVE）分支（2026-09-22 产品定案：与自己无关的组不返回，已解除成员不进入摘要与可见性判定，组关闭后成员全部解除因此不再返回）；主任务在前、来源任务按加入顺序；`groupRole`/`sourceKind`/`workStatus` 返回原始枚举由前端映射文案；`limit` 1～100、默认 20，`projectId` 只收窄范围；游标为服务端 HMAC 签名、绑定当前用户与筛选，TTL 15 分钟；非成员与不存在不返回 404 而是空页，字段校验失败统一 422 |
...
| `listProjectMembers` · `GET /api/v1/projects/{projectId}/members` | 401 | 允许（本项目活跃成员，ADR-039） | 404 | 404 | 401 | 允许 | 完整管理员 Session 或本项目活跃成员的认证 Session；返回项目成员完整历史（含 REMOVED）与脱敏 `name/avatarUrl/role`，不返回登录名/邮箱；不要求 CSRF 或幂等键；项目不存在或无权限统一 404；响应 `no-store` |
| `listProjectMemberUnfinishedTasks` · `GET /api/v1/projects/{projectId}/members/{userId}/unfinished-tasks` | 401 | 条件允许（同上） | 404 | 404 | 401 | 允许 | 同上；目标必须为 ACTIVE 成员，不存在、已移除或非成员统一 404；只返回当前 TODO 且 ACTIVE 的真实任务，提供改派所需 `rowVersion/moduleId/featureId` |
| `addProjectMember` · `POST /api/v1/projects/{projectId}/members` | 401 | 允许（本项目活跃成员，ADR-039） | 404 | 404 | 401 | 允许 | 项目未归档（ADR-035）；目标用户必须 ACTIVE；CSRF 与 `Idempotency-Key` 必填；新成员 `role='MEMBER'`；成员添加、审计、活动与通知在同一事务提交；重复活跃成员 409，停用/不存在用户 422；重放前重新验证当前认证、项目可写与操作者仍为本项目活跃成员 |
| `removeProjectMember` · `POST /api/v1/projects/{projectId}/members/{userId}/remove` | 401 | 条件允许（本项目活跃成员，ADR-039；目标为本项目 LEADER 时 409，须先由系统管理员转移/撤销） | 404 | 404 | 401 | 允许（目标为本项目 LEADER 时同样 409） | 项目未归档（ADR-035）；目标必须为 ACTIVE 成员；CSRF 与 `Idempotency-Key` 必填；可提交真实任务改派，未改派任务保留原负责人但成员立即失去处理权限与角色（REMOVED 行 role 回落 MEMBER）；任务改派、成员移除、审计与活动同一事务；`created_by` 不变；重放前重新验证当前认证、项目可写与操作者身份 |
| `setProjectMemberRole` · `POST /api/v1/projects/{projectId}/members/{userId}/role` | 401 | 403 `PROJECT_MEMBER_ROLE_FORBIDDEN`（ADR-039 起任命/撤销组长保留给系统管理员） | 404 | 404 | 401 | 允许（可设 MEMBER/LEADER，含转移组长） | ADR-033 / ADR-039 角色任命：请求 `role` 只能是 `MEMBER` / `LEADER`，其他值 422 `PROJECT_MEMBER_VALIDATION_FAILED`；项目未归档（ADR-035）；目标必须为 ACTIVE 成员；CSRF 与 `Idempotency-Key` 必填；LEADER 唯一性由部分唯一索引保证，冲突 409；审计 `project.member.role.set`、活动同事务；重放前重新验证当前认证、项目可写与操作者仍为系统管理员 |
| 项目、模块或功能归档/恢复 | 401 | 模块与功能归档/恢复允许（本项目活跃成员，ADR-039）；项目归档/恢复 403 | 404 | 404 | 401 | 允许 | 完整管理员 Session 或本项目活跃成员；写审计 |
...
| archiveFeature | 完整认证的系统管理员、本项目任意活跃成员（ADR-039） | 非成员（含已移除）404；项目未归档、父级可写（ADR-035）、功能 ACTIVE、原因和 If-Match；不要求功能下任务已归档（功能不参与任务归档前置校验） |
| restoreFeature | 同上 | 同上；功能 ARCHIVED；只恢复自身，不改下级状态 |

四条写接口重放前重查 Session/CSRF、当前项目授权、完整结果归属及当前管理员身份或（ADR-039 起）归档/恢复路由要求的项目内成员身份；父项目/模块归档拒绝重放。重复状态操作可重放原成功结果，不重复执行状态迁移。所有拒绝返回统一错误体，不泄露已存响应。真实测试入口见 [F-13 交审说明](f13-local-handoff.md)。
