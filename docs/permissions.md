...
9. `app_runtime` 搜索数据库权限按最小集显式授予，不得依赖 PUBLIC；普通查询只允许 `normalized_search_text &@~ app.pgroonga_query_escape($1)`。
