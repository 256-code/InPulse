...
| 任务卡片聚合关系批量查询（`listTaskGroupMemberships` · `GET /api/v1/task-groups/memberships`） | 401 | 允许 | 允许 | 允许 | 401 | 允许 | 只读批量查询；`taskIds` 为逗号分隔的 1～100 个正整数，数量、格式或重复校验失败统一 422；只返回当前用户实时可读项目内且属于 `ACTIVE` 聚合组的任务与组内角色（MAIN/SOURCE），无权、不存在或已解除的关系一律不入结果、不泄露存在性（无 404）；不要求 CSRF 或幂等键 |
