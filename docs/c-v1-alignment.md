...
冻结事实（裁决 §2、§3、Q-08 ~ Q-10）：`GET /api/v1/me/tasks`，参数只有 `cursor` / `limit` / `projectId` / `scopeType` / `workStatus` / `hasPublishedRecord`；负责人固定为当前用户；排序固定 状态分组（未完成 → 已完成 → 已取消）+ 紧急桶（标记紧急 → 已逾期 → 今/明日截止 → 其余）+ 优先级 + 遗留问题来源 + 截止时间 + 任务 ID（2026-09-18 由 [ADR-037](adr/ADR-037.md) 替代原 `id DESC`，待人工批准；2026-09-23 修订紧急桶并就遗留问题来源独立成级，游标版本 4 → 5）；`limit` 默认 20、上限 100。
...
2026-09-20 任务中心统计卡重定口径与「今日待办」筛选（产品定案，本地落库）：四张统计卡定为「今日待办 / 未完成 / 已完成 / 我创建的」。`MyTaskStats` 由 `myOpen`/`dueToday`/`overdue`/`completedThisMonth` 重定为 `todayTodo` + `todayTodoBreakdown`（`overdue`/`leftover`/`urgent`/`dueWithinDays`，四项各自独立计数、可以重叠，并集即 `todayTodo`）+ `myOpen` + `completed`（不再限本月）+ `created`；今日待办 / 未完成 / 已完成按负责人维度（assignee = 当前用户），`created` 按创建人维度（creator = 当前用户，无论指派给谁）；基准集合排除 `INVALID` / `CANCELED` 与历史来源分支，日界按 Asia/Shanghai 由 SQL 计算（7 天窗口为 `date_trunc('day', ...) + interval '7 days'`）。新增布尔筛选 `todayTodo`（`MyTasksQueryRequest` 与继承它的 `TaskCenterQuery` 同时支持）：只返回未完成且命中 逾期 ∪ 遗留来源 ∪ 标记紧急 ∪ 距截止 7 个日历日内 之一的任务，仍在分页 SQL 内先过滤后分页。上文第 57 行表格与 2026-09-11 段落中的 `stats` 字段列表由本条取代，其余 R-3 事实（路径、归属自指维度、游标的 HMAC 签名与筛选绑定、`limit` 1～100 默认 20、服务端计算日界）不变。前端 `MyTaskFilters` 增加 `todayTodo`，URL 参数 `today=1`，服务端适配器经生成客户端 `listTaskCenter` 透传（`projectId` 已选定时同样收窄）；四张卡分别切到 `mine+open+todayTodo`、`mine+open`、`mine+done`、`created+all`。证据见[测试矩阵](./test-matrix.md) 同名增量段。

2026-09-20 任务中心范围分段行移除与工具栏项目筛选（产品定案，本地落库）：`/tasks` 顶部范围分段行（我负责的 / 我创建的 / 按项目）整体删除——今日待办 / 未完成 / 已完成 即「我负责的」口径，我创建的由第 4 张卡承担；原「按项目」能力改为工具栏常驻项目筛选（`CalmSelect appearance="rich"`，首项「全部项目」，选项与遗留问题页同口径）；工具栏「未完成 / 已完成 / 全部」状态滑块一并删除，工作状态改由统计卡设定（`scope=mine|created` + `status=open|done|all`）。`filters.projectId` 由「仅 `scope=project` 或逾期钻取保留」改为常驻条件：`writeMyTaskFilters` 与 `toMyTasksV1Query` 只要选定项目就写 URL 并下发 `projectId`，任务聚合组与四张统计卡随之收窄（R-3 统计基准集合本就只受 `projectId` 影响）。URL 参数 `scope` 与非管理员回落 `mine` 的降级语义不变，页面不再渲染任何范围 tab；`TaskCenterPageView` 的 `isAdmin` 属性随范围 tab 一并删除。服务端契约、权限与行为未改。证据见[测试矩阵](./test-matrix.md) 同名增量段。

2026-09-20 任务中心统计卡选中态、默认落地「今日待办」与列表标题行删除（产品定案，本地落库）：任务中心打开即落在「今日待办」，因此四张统计卡要有选中态——当前生效的那张用深一档描边 + 浅底（`.stat-card-selected`）+ `aria-pressed` 表达，选中项由筛选反推（`created+all` / `mine+done` / `mine+open` 且 `todayTodo !== false` / `mine+open` 且 `todayTodo === false` 分别对应我创建的 / 已完成 / 今日待办 / 未完成，组合对不上则不选中）。「今日待办」只在工作状态为「未完成」时有意义（服务端 `todayTodo` 与 `DONE` / `CANCELED` 求交恒为空），URL 与请求都按此归一：不带 `today` 且 `status=open` 即今日待办（`DEFAULT_MY_TASK_FILTERS.todayTodo = true`），「未完成但不限今日」写 `today=0`，`status=done|all` 既不下发 `todayTodo` 也不写该参数——因此 `?status=done` 这类旧链接不会被静默套上今日筛选而变空。列表区块不再重复「未完成 / n 项 · 服务端按任务编号倒序」两行文字，工作状态只由统计卡选中态表达，区块只留展示方式图标（`.task-list-mark`）与空态；今日待办空态改说「今天没有待办任务」并说明它比未完成更窄，避免看起来像漏了任务。工具栏项目筛选去掉与下拉内容重复的「项目」文字标签，无障碍定位仍由 `aria-label="项目"` 提供。服务端契约、权限与行为未改。证据见[测试矩阵](./test-matrix.md) 同名增量段。
