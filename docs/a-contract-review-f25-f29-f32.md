...
| Q-10 | 固定 `id DESC`（2026-09-18 由 [ADR-037](adr/ADR-037.md) 替代，改按 状态分组 + 紧急桶 + 优先级 + 遗留问题来源 + 截止时间 + 任务 ID；2026-09-23 起紧急桶收窄为 标记紧急 → 已逾期 → 今/明日截止 → 其余，遗留问题来源独立成级） | 不提供 `sort`；游标沿用 C-006（`MY_TASKS` 载荷按 ADR-037 扩展为多列 keyset，2026-09-23 起 8 段、版本 5） |
...
| 参数 | A 裁决 | 说明 |
| --- | --- | --- |
| `priority` | 接受：单值 | 与 `workStatus` 正交；`dueAt` 级别的排序由 [ADR-037](adr/ADR-037.md) 固定为「状态分组 + 紧急桶 + 优先级 + 截止时间 + 任务 ID」（原 `id DESC` 已替代；2026-09-23 起为「状态分组 + 紧急桶（四档）+ 优先级 + 遗留问题来源 + 截止时间 + 任务 ID」） |
