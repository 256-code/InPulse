...
| Q-10 | 固定 `id DESC`（2026-09-18 由 [ADR-037](adr/ADR-037.md) 替代，改按 状态分组 + 紧急桶 + 优先级 + 遗留问题来源 + 截止时间 + 任务 ID；紧急桶为 标记紧急 → 已逾期 → 今/明日截止 → 其余 四档，遗留问题来源是优先级之后的独立一级，2026-09-24 复核确认） | 不提供 `sort`；游标沿用 C-006（`MY_TASKS` 载荷按 ADR-037 扩展为多列 keyset，2026-09-24 起 8 段、版本 7） |
...
| Q-10 | 固定 `ORDER BY t.id DESC`，不提供 `sort` | 现有索引可命中 `ORDER BY t.id DESC`，`updated_at` 无可用索引（C 端口提案 §1.4）；游标约定沿用 C-006 | R-3 的 `limit` 默认 20、上限 100；游标签名绑定 actor 与筛选条件，TTL 15 分钟；无效、过期或越界返回 `422`。2026-09-18 由 [ADR-037](adr/ADR-037.md) 替代：排序改为固定「状态分组 + 紧急桶 + 优先级 + 遗留问题来源 + 截止时间 + 任务 ID」（紧急桶为 标记紧急 → 已逾期 → 今/明日截止 → 其余 四档，遗留问题来源是优先级之后的独立一级），`MY_TASKS` 游标载荷扩展为多列 keyset 且缺失该键的旧载荷按无效游标 `422`（待人工批准）；2026-09-24 复核：同日早些时候「遗留问题来源并回紧急桶、游标版本 6、载荷 7 段」的临时口径已作废，现行游标版本 7、载荷 8 段，v5 / v6 旧键同样整版拒绝 |
...
| 参数 | A 裁决 | 说明 |
| --- | --- | --- |
| `priority` | 接受：单值 | 与 `workStatus` 正交；`dueAt` 级别的排序由 [ADR-037](adr/ADR-037.md) 固定为「状态分组 + 紧急桶 + 优先级 + 遗留问题来源 + 截止时间 + 任务 ID」（原 `id DESC` 已替代；紧急桶四档，遗留问题来源为优先级之后的独立一级，2026-09-24 复核确认） |
