...
| [ADR-034](ADR-034.md) | Accepted | 项目归档申请—审核（双人同意）、任务与功能归档及父级归档前置校验；角色口径经 ADR-039 修订 |
| [ADR-035](ADR-035.md) | Accepted | 项目生命周期四态（未开始 / 进行中 / 维护中 / 已归档）、粘性标记与项目写入口口径；角色口径经 ADR-039 修订 |
...
| [ADR-041](ADR-041.md) | Accepted | 原始审计读取留痕按「查看」计数（进入与切换对象各一条） |
