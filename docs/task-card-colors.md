...
产品反馈「任务卡片不够醒目」后，卡片从浅色底改为按优先级铺实色（D2 方案）；「高」的底色经两轮候选对比后定为明黄 `#ffdb4d` + 深棕字。以下色值取自 `apps/web/src/styles/design-system.css` 末尾的「任务卡片醒目配色」块，选择器为 `:is(.calm-task-card, .tb-card).tone-prio-*`，比列表行用的 `.tone-prio-*` 高一级，因此只作用于卡片。「已完成」在 2026-09-22 末轮收成一套青碧色值（三处表面同值），见下文《「已完成」：青碧一套色值》。2026-09-22 三次定案后，已逾期与马上到期**不再整卡换色**（产品要求「逾期的不搞特殊了，原本的优先级是什么就呈现什么颜色，只是排序靠前，比紧急低一档」）：卡片只剩「紧急 / 高 / 普通 / 已完成 / 已取消」五档 + 遗留问题锈红，见下文《逾期与马上到期：不再整卡换色》。2026-09-23 起「低」（LOW）档位整体下线（[ADR-041](./adr/ADR-041.md)），优先级只剩 `URGENT / HIGH / NORMAL` 三档，本文中凡出现「低」的表格行、色值与说明均已删除，历史变更记录保留原文。由遗留项转换而来的任务另有一档自己的色：整卡锈红 `#8a2b06`，见下文《「遗留问题」：整卡锈红》。2026-09-23 二次定案（产品要求「紧急颜色不变，优先级高改为蓝色的，优先级普通改为白色的」）：**紧急仍是红 `#ce342b`，「高」换成原「普通」的蓝 `#337ee6` + 白字，「普通」换成原「低」的白底卡 `#ffffff` + 深色正文**——明黄 `#ffdb4d` 与深棕字 `#3f2d00` 自此退出。2026-09-23 三次定案（产品要求「优先级高的这个卡片颜色改为这个图里的黄色」，附图为品牌 Logo 的盾形金色，逐像素采样得 `#fdc106`）：**那支蓝只在位半天即退出，「高」改为金黄 `#fdc106` + 深棕字 `#3f2d00`（8.07:1），描边 `#daa605`、悬停 `#e4ae05`；「普通」白底卡与紧急 / 已完成 / 已取消 / 遗留问题锈红全部不变**。2026-09-23 四次定案（产品要求「把这个模式下的颜色也改一改，和卡片对应」，指任务中心的列表视图）：**列表行改取卡片同族色——色条直接用卡片实色（紧急 `#ce342b`、「高」`#fdc106`），底色是卡片色淡到极浅的一调（紧急 `#fdf2f1`、「高」`#fff8de`），标题是同色系深字（紧急 `#a52a22` 6.48:1、「高」`#6b4c00` 7.42:1）；卡片本身的色值不动**。2026-09-23 五次定案（产品要求「颜色再深一点」）：**列表行 / 表格行整体加深一档**，卡片实色不动——紧急色条 `#be3028` / 底 `#fae0dd` / 字 `#8f1f18`（7.06:1）、高色条 `#e6b005` / 底 `#fff3c9` / 字 `#5f4500`（8.09:1），普通 / 已完成 / 已取消 / 遗留问题四档同批加深（已取消压得更冷一档，避免与「普通」行分不出来）。同一套色值同步到列表行、下拉圆点、优先级徽章与聚合组卡，见下文《优先级圆点》与文末变更历史。

## 五种程度的卡片取值

![任务中心卡片实色（样图生成于 2026-09-22，已被 2026-09-23 的定案取代：含已下线的「低」档，且「高」还是明黄 `#ffdb4d`、「普通」还是蓝 `#337ee6`；当前「高」是金黄 `#fdc106`、「普通」是白底卡）](./assets/task-card-colors/cards-overview.png)

| 程度 | tone 类名 | 卡片底色 | 描边 | 悬停底色 | 卡内文字 | 文字对比度 |
| --- | --- | --- | --- | --- | --- | --- |
| 紧急 | `tone-prio-urgent` | `#ce342b` | `#b12c25` | `#b92e27` | `#ffffff` | 5.09:1 |
| 高 | `tone-prio-high` | `#fdc106` | `#daa605` | `#e4ae05` | `#3f2d00` | 8.07:1 |
| 普通 | `tone-prio-normal` | `#ffffff` | `#d9e0e7` | `#f5f8fa` | 继承深色正文 | 常规 |
| 已完成 | `tone-prio-done` | `#e1f2f2` | `#bcdddf` | `#d6edee` | `#1d5b62` | 6.67:1 |
| 已取消 | `tone-prio-canceled` | `#f2f5f8` | `#d5dde4` | `#e8edf1` | 继承深色正文 | 常规 |

说明：

- 对比度为白字或深棕字对实色底的计算值。2026-09-23 三次定案后，**实色卡里不再有低于 AA 的 4.5:1 的档位**：紧急白字 5.09:1、「高」深棕字 8.07:1、「遗留问题」白字 8.65:1、「已完成」深青字 6.67:1；「普通」白底卡与「已取消」浅灰底都是页面深色正文，属常规。二次定案期间「高」曾接原「普通」的蓝 + 白字（3.99:1）而落在 AA 之下，同日晚些时候三次定案把「高」改回浅底深字，这条取舍随之消失。更早的「高」明黄底 `#ffdb4d` + 深棕字 `#3f2d00`（9.76:1，记为 9.8:1）是当年为解掉土黄 `#c47b00` 白字只有 3.4:1 而定。「高」原本用土黄 `#c47b00` + 白字只有 3.4:1，换成明黄 + 深棕字后达到 9.8:1；「已完成」原本用 `#2f9a68` + 白字只有 3.5:1、换成 `#12875f` 又偏沉，最终定为极浅嫩绿 `#a8e6b0` + 深绿字 `#2f4031`（7.7:1），2026-09-22 逐轮放淡（`#a8e6b0` → `#cdf1d3`）；末轮按聚合组配色候选样板上拍板的 D「青碧」改成青碧底 `#e1f2f2` + 深青字 `#1d5b62`（6.67:1），三处表面不再分色，见下文《「已完成」：青碧一套色值》。
- 「已取消」用浅灰底、不铺优先级实色，靠 2px 描边与页面底色区分。与它成对的「低」白底卡随 2026-09-23 档位下线一并删除（[ADR-041](./adr/ADR-041.md)），同一套白底样式在同日二次定案里由「普通」接手（`#ffffff` / 描边 `#d9e0e7` / 悬停 `#f5f8fa`）——白卡在浅色页底上靠 2px 描边划界。
- 卡片统一 2px 描边，且不再显示左侧色条（`::before { display: none }`）——底色本身已表达优先级，多一条深色竖边在实色卡上显脏。2026-09-22 起**实色卡的描边统一取「底色压暗 14%」**（紧急 `#ce342b`/`#b12c25`、「高」`#fdc106`/`#daa605`，「遗留问题」锈红 `#8a2b06`/`#772505` 同口径），边/底对比度稳定在 1.29–1.30；浅底族与中性族（「普通」白 `#ffffff`/`#d9e0e7`、已完成 `#e1f2f2`/`#bcdddf`、已取消 `#f2f5f8`/`#d5dde4`）各自成对，其中已完成的 `#e1f2f2`/`#bcdddf` 是产品 2026-09-22 在聚合组配色候选样板上选定的 D「青碧」。
...
白字组（紧急 / 遗留问题）与深字组（高 / 已完成）除正文外还统一处理编号、meta、徽章、日期与勾选；两组的颜色统一由 `--task-ink-*` 变量驱动，取值写在各档自己的卡片色值块里（「高」= `#3f2d00` 深棕、已完成 = `#1d5b62` 深青）。2026-09-23 三次定案后「高」重新回到深字组（二次定案期间它曾是白字组的实色蓝卡）；「普通」白底卡两组都不写 `--task-ink-*`，直接继承页面深色正文：

| 元素 | 白字组（紧急 / 遗留问题） | 深字组（高 / 已完成） |
| --- | --- | --- |
...
白字组 / 深字组之外的两档不在这张表里：「普通」白底卡与「已取消」浅灰卡都不覆盖卡内文字，直接继承页面深色正文与默认的徽章底色，与它们接替的旧「低」白卡同一口径。
...
![聚合组卡片的底色跟随未完成分支的最高优先级（样图生成于 2026-09-22）](./assets/task-card-colors/group-states.png)
...
| 未开始 / 进行中，最高一档 = 高 | `tone-prio-high` | `#fdc106` | `#daa605` | `#e4ae05` | `#3f2d00`（8.07:1） |
| 未开始 / 进行中，最高一档 = 普通 | `tone-prio-normal` | `#ffffff` | `#d9e0e7` | `#f5f8fa` | 深色正文 |
| 已完成（分支全部收尾） | `tone-prio-done` | `#e1f2f2` | `#bcdddf` | `#d6edee` | `#1d5b62`（6.67:1） |
| 已关闭且无分支（无事实可派生） | `tone-prio-canceled` | `#f2f5f8` | `#d5dde4` | `#e8edf1` | 深色正文 |

说明：

- 只派生一个事实：`describeTaskGroup` 取未完成（`workStatus === "TODO"`）分支里最高的一档（紧急 > 高 > 普通）；该分支完成后自动落到第二高，全部分支收尾后按「已完成」呈现并隐藏优先级徽章。已取消与已完成一样算收尾，否则被取消的分支会永远压住组优先级。
...
- 未完成组的排序（2026-09-22 产品要求「当组合任务为未完成状态（未开始或进行中）排序按照合并任务以内部未完成任务中的最高优先级」，同日追加「（它们）同样是一个优先级的，按照截止日期从近到远排序」）：组卡不再单独排一遍再追加到任务卡后面，而是与任务卡共用 `gridEntries` 这一把尺子——状态分组（未完成 0 / 已完成 1 / 已取消 2）→ 紧急桶（标记紧急 0 / 已逾期 1 / 今-明日截止 2 / 其余 3）→ 优先级档位（组取未完成分支最高一档）→ 遗留问题来源（组卡按固定 1 参与，与任务卡共用同一级位置）→ 截止时间近到远（组取未完成分支中最早的一条，未设截止排最后）。各项都取自服务端 `apps/api/src/modules/tasks/task-list-order.ts` 的排序键，组卡的各项都从「未完成分支」派生；卡片视图与列表视图同一顺序，同键保持稳定次序（任务在前、组保持 R-7 顺序）。`visibleGroups` 现在只做「未完成 / 已完成」归属筛选。
- 组卡不参与日期档（2026-09-22 三次定案「逾期的不搞特殊了，原本的优先级是什么就呈现什么颜色」）：`describeTaskGroup` 的 `tone` 只由派生优先级决定，右下角的截止文案仍按未完成分支中最早的一条显示（与任务卡片同文案，如「已逾期 9月16日」），不会把整卡染红。同日早些时候先落的「组卡底走截止档（未完成分支最早一条已逾期 → `tone-prio-overdue`、今天到期 → `tone-prio-soon`）」按本条口径作废，本地真实数据 `K123-TG-1`（两条分支都未完成、最早截止已逾期）因此回到派生优先级的「普通」档（当日是蓝；2026-09-23 二次定案起该档改白，见文末变更历史）。
- 卡内文字不再为组卡单写一套：组卡本身带 `.calm-task-card`，白字组（紧急 / 遗留问题）与深字组（高 / 已完成）的既有规则直接命中底部那一排的徽章与截止文案，因此不需要额外的组卡规则。
...
- 排序：服务端 `apps/api/src/modules/tasks/task-list-order.ts` 的紧急桶（仅未完成参与，命中第一个）把未完成任务排成「标记紧急(0) → 已逾期(1) → 今/明日截止(2) → 其余(3)」，紧急桶之后是优先级，再之后是独立的「遗留问题来源(0/1)」一级——它自 2026-09-23 起不再属于紧急桶，2026-09-24 复核后维持该结构（产品口径「遗留问题只会在同优先级里面高一点」），因此遗留问题来源只在同优先级任务内提前，不越过更高优先级的任务。游标键版本随历次重排由 1 升到 2、同日再升到 3，2026-09-23 由 4 升到 5（遗留问题来源独立成级、载荷 8 段）；2026-09-24 两条并行开发线合并时曾由 5 升到 6 把遗留问题来源并回紧急桶（该口径已作废），同日复核后由 6 升到 7、载荷回到 8 段，旧游标一律按无效游标拒绝。
...
- 卡片一律按任务自己的优先级取色：「高」逾期仍是高蓝、「普通」逾期仍是普通白；「遗留问题」来源仍是锈红（见下文《「遗留问题」：整卡锈红》）。
...
- 色值与「遗留问题」徽章（`.badge-leftover`）同源：卡片底色、列表行色条与文字、徽章实底都是 `#8a2b06`；描边按既定口径取「底色压暗 14%」（`#772505`），悬停取压暗 10%（`#7c2705`）。白字对比度 8.65:1，高于其余实色卡（紧急 5.09:1；同日四次定案后「高」是浅底金卡 + 深棕字，不再参与白字对比），因此卡内 11–12px 的小字在这一档上反而不存在 AA 取舍。
- 覆盖次序（`task-tone.ts` 的 `taskToneOf(priority, workStatus, hasLeftoverSource)`，2026-09-22 三次定案后只剩三个参数）：已完成 → 已取消 → 紧急 → **遗留问题** → 优先级。完成态与最高优先级身份都不被来源标记盖掉——遗留项转来的紧急卡仍是紧急红 `#ce342b`；只有「高 / 普通」两档会被来源标记接管成锈红。因此遗留项的卡片恒为红系两档之一（紧急红或锈红），不会再出现蓝卡或白卡；这张卡是不是遗留项由卡上的「遗留问题」徽章与卡片底色共同表达。
...
- 实测（无头 Chromium 1440 宽，本地 dev 5173，真实数据只读）：任务中心的 `A-5 残余：5 年峰值模型定稿与 1.2 倍条件人工复核`（`hasLeftoverSource`、普通、未逾期）得 `calm-task-card tone-prio-leftover`、背景 `rgb(138, 43, 6)`、描边 `rgb(119, 37, 5)`、文字 `rgb(255, 255, 255)`，卡内两枚徽章（普通 / 遗留问题）都是 `rgba(255, 255, 255, 0.18)` 底 + 白字；同页相邻的 `A-6` 仍是高明黄 `rgb(255, 219, 77)`、普通任务仍是普通蓝 `rgb(51, 126, 230)`、逾期组卡仍是 `rgb(179, 38, 30)`（本行是 2026-09-22 的当日实测事实；2026-09-23 三次定案后「高」为金黄 `rgb(253, 193, 6)`、「普通」为白 `rgb(255, 255, 255)`，见文末变更历史）。切「列表」视图后该行类名为 `tone-prio-leftover`、底色 `rgb(251, 238, 233)`、标题 `rgb(138, 43, 6)`。
...
![任务看板卡片实色（样图生成于 2026-09-22，已被 2026-09-23 的定案取代：含已下线的「低」档，且「高」还是明黄 `#ffdb4d`、「普通」还是蓝 `#337ee6`；当前「高」是金黄 `#fdc106`、「普通」是白底卡）](./assets/task-card-colors/board-overview.png)

## 列表行与表格行保持浅色

项目任务面板列表（`.feature-list-table tbody tr`）、任务中心列表（同一套 `.tone-prio-*`）与看板列表视图（`.tb-row--tone-*`）继续用 `.tone-prio-*` 的浅色底与左侧色条：表格里深底配深字读不出来，因此不做实色卡。2026-09-23 四次定案（产品要求「把这个模式下的颜色也改一改，和卡片对应」）改了「同一程度在卡片与列表行的色值不同属于刻意选择」这条旧口径：**列表行现在按卡片色派生——色条直接用卡片实色，底色是卡片色淡到极浅的一调，标题是同色系深字**。紧急行的色条因此由压暗的 `#c0453f` 换成卡片红 `#ce342b`、标题由发灰的 `#8f3f3f` 换成 `#a52a22`（6.48:1）；「高」行由土金 `#c9821a` + 米黄 `#fdf8ee` + 棕字 `#7d5310`（与卡片金黄差得远、在浅色表格里发脏）换成金条 `#fdc106` + 暖金底 `#fff8de` + 深金字 `#6b4c00`（7.42:1）。已完成（`#1d5b62` / `#e1f2f2`）与遗留问题（`#8a2b06` / `#fbeee9`）两档本来就与卡片同值，无需改动。同日五次定案又把六档整体加深一档（中间值见文末变更历史）；六次定案（产品要求「普通的任务在列表模式下改为蓝色边边，白色底」）再把「普通」行由近白中性底 `#eef2f6` 换成**蓝色色条 `#1467d8` + 白色底 `#ffffff`**：白底行靠左侧 3px 蓝边与单元格分隔线（`#edf1f6`）分界，行内文字仍取中性深色 `#51606e`（对白底 6.6:1），悬停 `#f2f7fe`、描边 `#dbe6f5`；卡片侧刻意不跟随——任务中心的「普通」白底卡与看板卡片继续用自己的中性灰竖条 `#8b99a7`（见下面卡片实色块的 `--task-c`）。七次定案（产品要求「我想 p1 用 p2 的颜色」）再把「已完成」一档的列表行拉回与卡片同值——底 `#e1f2f2`、悬停 `#d6edee`、描边 `#bcdddf`，色条与标题仍是 `#17535a`；这一档因此不再遵守五次定案「列表行比卡片深一档」的规律（卡片侧本来就是这组值，行为不变），其余五档仍比卡片深一档。看板列表视图的「遗留问题」行单独用 `.tb-row--leftover`（`--tb-row-accent` / `--tb-row-fg` `#8a2b06`、`--tb-row-bg` `#fbeee9`、`--tb-row-hover` `#f7e3da`），与其他行配色块并列。

| 程度 | 色条 | 底色 | 悬停底色 | 描边 | 文字 |
| --- | --- | --- | --- | --- | --- |
| 紧急 | `#be3028` | `#fae0dd` | `#f5cfcb` | `#eec4bf` | `#8f1f18` |
| 高 | `#e6b005` | `#fff3c9` | `#fbe7a6` | `#efd88c` | `#5f4500` |
| 普通 | `#1467d8` | `#ffffff` | `#f2f7fe` | `#dbe6f5` | `#51606e` |
| 已完成 | `#17535a` | `#e1f2f2` | `#d6edee` | `#bcdddf` | `#17535a` |
| 已取消 | `#96a4b2` | `#e9edf1` | `#dbe2e9` | `#ccd5de` | `#5b6875` |
| 遗留问题 | `#7c2705` | `#f7dfd3` | `#f2cdbd` | `#e8bda8` | `#7c2705` |

- 标题对底色的对比度（五次定案加深，六次 / 七次定案后的当前版）：紧急 `#8f1f18` / `#fae0dd` 7.06:1、「高」`#5f4500` / `#fff3c9` 8.09:1、普通 `#51606e` / `#ffffff` 6.6:1（由灰底时的 5.75:1 略升）、已完成 `#17535a` / `#e1f2f2` 7.42:1（七次定案改用卡片青碧后由 7.06:1 略升）、已取消 `#5b6875` / `#e9edf1` 5.03:1，都高于 AA 的 4.5:1（四次定案时已取消只有 3.66:1，本轮一并修掉）。色条与底色同族，对比度天然低，所以识别不靠色条单点：行内还有优先级徽章与同色系标题。
- 看板列表视图的行变量（`.tb-row--tone-red` / `--tone-amber` / `--tone-gray` / `--done` / `--canceled` / `--leftover`）与上面六档同值。看板优先级标签 `.tb-prio--red` / `--amber` / `--gray` 自五次定案起跟随该档的行文字色（`#8f1f18` / `#5f4500` / `#5f6d7a`）：底色加深后，四次定案时的灰标签 `#8b99a7` 对 `#eef2f6` 只剩 2.59:1、「普通」行的标签读不出来。六次定案把「普通」行底色换成白之后，`.tb-prio--gray` 的 `#5f6d7a` 对白底是 5.03:1，无需再动；七次定案把 `.tb-row--done` 与表格行一起改用卡片青碧（底 `#e1f2f2`、悬停 `#d6edee`、色条与文字 `#17535a`），看板与任务中心仍保持同值；同时 `.tb-row--tone-gray` 与 `.tone-prio-normal` 一样改成蓝边白底 `#1467d8` / `#ffffff` / `#f2f7fe` / `#51606e`——类名里的 gray 来自 `task-tone` 的色调映射（同一个 tone 值同时喂徽章与看板行类名），不再描述颜色。旧「普通」蓝卡时代留下的 `.tb-row--tone-blue` / `.tb-prio--blue` 两个块自三次定案起已没有调用方（`priorityMarkOf('NORMAL')` 返回 `gray`），留作历史取值。

## 优先级圆点

新建 / 编辑任务与遗留问题转换等处的优先级下拉圆点取与卡片同源的色值（`apps/web/src/features/common/priority-select-option.ts`）。2026-09-23 三次定案后：实色卡直接取卡片实色（紧急 `#ce342b`）、浅底卡取同族深版（「高」的金黄卡配 `#8a6e00`），白底卡（「普通」）不能配白色圆点——8px 白点压在白底弹层里看不见，因此沿用旧「低」白卡的中性灰 `#a0adb9`：

| 优先级 | 卡片实色 | 下拉圆点 |
| --- | --- | --- |
| URGENT | `#ce342b` | `#ce342b` |
| HIGH | `#fdc106` | `#8a6e00`（金黄卡取同族深版） |
| NORMAL | `#ffffff` | `#a0adb9`（白卡取中性灰） |

> 配对口径：实色卡取实色、浅底卡取同族深版、白底卡取中性灰。2026-09-23 起「低」档位与前文所有低档色值一并删除；未知优先级按「普通」的中性灰 `#a0adb9` 兜底。同日二次定案曾把「高」的圆点由明黄深版 `#8a6e00` 换成 `#337ee6`（与当时的蓝卡同值），三次定案随「高」改金黄又换回 `#8a6e00`。

## 实现位置

- `apps/web/src/styles/design-system.css`「任务卡片醒目配色」块：卡片实色、描边、悬停与卡内文字（实色红只剩紧急一支）；同文件上方的 `.tone-prio-*` 供列表行 / 表格行使用，2026-09-23 四次定案后按卡片色派生（色条取卡片实色、底色取卡片色的极浅调、文字取同色系深字）；看板列表行 `.tb-row--tone-*` 与看板优先级标签 `.tb-prio--*` 同批同步（2026-09-23 五次定案整体加深一档后，标签改为跟随该档的行文字色）；末尾「红档收敛成一支」说明块记录口径与对比度。
...
- 优先级 → 徽章色调的映射共三处，必须同源：`apps/web/src/features/common/task-tone.ts` 的 `taskPriorityBadgeTone`（任务中心与聚合组明细）、`apps/web/src/features/tasks/TasksPanel.tsx` 的 `priorityTone`（项目任务面板）、`apps/web/src/features/task-board/task-board-format.ts` 的 `priorityMarkOf`（任务看板）。`priorityMarkOf` 的色调同时决定看板列表视图行的 `tb-row--tone-*` 类名，因此它的「普通」决定行底色走 `.tb-row--tone-gray`；2026-09-23 三次定案把后两处的「普通」由 `blue` 补齐为 `gray`，与白底卡同调。
...
- `apps/api/src/modules/tasks/task-list-order.ts`：统一排序键的紧急桶 / 遗留问题来源 / 完成时间倒序与游标键版本（`TASK_LIST_SORT_KEY_VERSION`，2026-09-22 由 1 升到 2、同日再升到 3：新增「完成时间倒序」一级，已完成分组按 `completed_at` 从晚到早，未完成 / 已取消用 `'-infinity'` 归一后不参与比较；游标载荷由 6 段变 7 段；2026-09-23 随「低」档位下线由 3 升到 4：优先级序号由 0/1/2/3 收窄为 0/1/2，旧游标整版拒绝；2026-09-23 由 4 升到 5：紧急桶收窄为「标记紧急(0) → 已逾期(1) → 今/明日截止(2) → 其余(3)」，遗留问题来源独立成优先级之后的一级，载荷 8 段；2026-09-24 曾由 5 升到 6 把遗留问题来源并回紧急桶第 2 档、载荷回到 7 段——**该口径已作废**；同日复核后由 6 升到 7：恢复「紧急桶四档 + 遗留问题来源独立一级」，载荷回到 8 段）。
...
| 2026-09-23 | 删除「低」（LOW）档位（[ADR-041](./adr/ADR-041.md)）：卡片表与组卡表删掉「低」行、列表行表的「低」行与优先级圆点表的 `LOW` 行一并删除，`design-system.css` 的 `tone-prio-low` 三处定义（列表行浅色变量、行标题选择器、卡片实色块）同步移除；`task-tone.ts` 的 `TaskToneName` / `PRIORITY_LABELS` / `PRIORITY_BADGE_TONES` 与 `priority-select-option.ts` 的圆点表删掉低档，未知兜底由「低」改为「普通」（`normal` / `#337ee6`）；下拉（新建/编辑任务、遗留项转任务、任务看板筛选、任务中心筛选）不再有「低」选项。数据库侧 `0022_drop_low_priority.sql` 把存量 `LOW` 归一到 `NORMAL` 并收紧 `tasks_priority_check`；`TASK_LIST_SORT_KEY_VERSION` 由 3 升到 4。样图未重新生成，仍含历史「低」卡。 |
| 2026-09-23 | 「高」改蓝、「普通」改白（产品要求「紧急颜色不变，优先级高改为蓝色的，优先级普通改为白色的」）：卡片实色「高」由明黄 `#ffdb4d` / `#d6b841` / `#edcc48` + 深棕字 `#3f2d00` 改为原「普通」的蓝 `#337ee6` / `#2c6cc6` / `#3076d8` + 白字，「普通」由蓝改为原「低」的白底卡 `#ffffff` / `#d9e0e7` / `#f5f8fa` 并继承深色正文；`design-system.css` 的白字组由（紧急 / 普通 / 遗留问题）改为（紧急 / 高 / 遗留问题），深字组只剩「已完成」，「普通」不再写 `--task-ink-*`。列表行 / 表格行同口径互换：「高」吃原「普通」的 `#1467d8` / `#f0f5fd` / `#e1ecfa` / `#cfdff7` / `#16528f`，「普通」吃原「低」的 `#a0adb9` / `#f6f8fa` / `#ebeff3` / `#dfe5ea` / `#5f6d7a`。下拉圆点：HIGH `#8a6e00` → `#337ee6`、NORMAL `#337ee6` → `#a0adb9`，未知值兜底同改中性灰；优先级徽章色调 HIGH `amber` → `blue`、NORMAL `blue` → `gray`（`task-tone.ts`）。无头 Chromium 注入真实样式表实测计算值：`tone-prio-urgent` 底 `rgb(206, 52, 43)` / 白字、`tone-prio-high` 底 `rgb(51, 126, 230)` / 描边 `rgb(44, 108, 198)` / 白字、`tone-prio-normal` 底 `rgb(255, 255, 255)` / 描边 `rgb(217, 224, 231)` / 标题 `rgb(36, 61, 84)`，已完成 / 已取消 / 遗留问题三档不变。样图未重出，仍含旧「高」明黄与旧「普通」蓝。 |
| 2026-09-23 | 「高」改金黄（产品要求「优先级高的这个卡片颜色改为这个图里的黄色」，附图取自品牌 Logo 的盾形金色，逐像素采样得 `#fdc106`）：卡片实色「高」由二次定案的蓝 `#337ee6` / `#2c6cc6` / `#3076d8` 改为 `#fdc106` / `#daa605` / `#e4ae05`，卡内文字从白字回到深棕字 `#3f2d00`（8.07:1），九个 `--task-ink-*` 变量重新写回该色值块；`design-system.css` 的白字组由（紧急 / 高 / 遗留问题）改回（紧急 / 遗留问题），深字组由「已完成」一张扩成（高 / 已完成）。列表行 / 表格行同步回到金黄族：色条 `#c9821a`、底色 `#fdf8ee`、悬停 `#faeed7`、描边 `#f0dfc0`、文字 `#7d5310`。下拉圆点 HIGH 由 `#337ee6` 回到同族深版 `#8a6e00`；优先级徽章色调 HIGH 由 `blue` 回到 `amber`，并把二次定案漏掉的两处「普通」徽章补齐为 `gray`（任务看板 `priorityMarkOf` 与项目任务面板 `priorityTone`，与白底卡同调），看板列表视图「普通」行随之由 `tb-row--tone-blue` 改为 `tb-row--tone-gray`。紧急 `#ce342b`、「普通」白卡、已完成青碧、已取消与遗留问题锈红均不变；三次定案后实色卡里不再有低于 AA 的档位。无头 Chromium 注入真实样式表实测计算值：`tone-prio-high` 底 `rgb(253, 193, 6)` / 边 `rgb(218, 166, 5)` / 标题 `rgb(63, 45, 0)`、卡内徽章 `rgba(63, 45, 0, 0.12)` + 深棕字、金黄列表行底色 `rgb(253, 248, 238)` / 色条 `rgb(201, 130, 26)` / 标题 `rgb(125, 83, 16)`；紧急 `rgb(206, 52, 43)`、白卡 `rgb(255, 255, 255)`、已完成 `rgb(225, 242, 242)`、已取消 `rgb(242, 245, 248)`、遗留问题 `rgb(138, 43, 6)` 不变。样图未重出，仍含旧「高」与旧「普通」。 |
| 2026-09-23 | 列表行按卡片色派生（产品要求「把这个模式下的颜色也改一改，和卡片对应」，指任务中心 `/tasks?view=list` 的列表视图；附图为列表视图截图，「高」行当时是米黄底 + 土金色条 + 棕字，与金黄卡差得远、在浅色表格里发脏）：口径由「同一程度在卡片与列表行的色值不同属于刻意选择」改成「列表行按卡片色派生」——**色条直接用卡片实色、底色是卡片色淡到极浅的一调、标题是同色系深字**。`design-system.css` 上方的 `.tone-prio-urgent` 色条 `#c0453f` → `#ce342b`（卡片红）、标题 `#8f3f3f` → `#a52a22`、悬停 `#fbe7e5` → `#fbe4e2`，底色 `#fdf2f1` 与描边 `#f2d3d0` 不动；`.tone-prio-high` 由 `#c9821a` / `#fdf8ee` / `#faeed7` / `#f0dfc0` / `#7d5310` 换成 `#fdc106`（卡片金）/ `#fff8de` / `#fdecbf` / `#f4e2a6` / `#6b4c00`。看板列表视图的 `.tb-row--tone-red` / `.tb-row--tone-amber` 同值同步；看板优先级标签 `.tb-prio--red` `#c0453f` → `#c02b23`（5.30:1）、`.tb-prio--amber` `#c9821a` → `#8a6e00`（与下拉圆点同值，对行底 2.96:1 → 4.58:1，修掉旧值在 10px 小字上不达 AA 的欠账）。「普通」「已完成」「已取消」「遗留问题」四档不变。无头 Chromium 注入真实样式表实测计算值：紧急行底 `rgb(253, 242, 241)` / 色条 `rgb(206, 52, 43)` / 标题 `rgb(165, 42, 34)`，「高」行底 `rgb(255, 248, 222)` / 色条 `rgb(253, 193, 6)` / 标题 `rgb(107, 76, 0)`，普通 `rgb(246, 248, 250)` / `rgb(160, 173, 185)` / `rgb(95, 109, 122)`，已完成 `rgb(225, 242, 242)` / `rgb(29, 91, 98)`，已取消 `rgb(245, 247, 249)` / `rgb(170, 183, 196)`，遗留问题 `rgb(251, 238, 233)` / `rgb(138, 43, 6)` 与卡片同值。卡片侧无改动，样图未重出。 |
| 2026-09-23 | 列表行整体加深一档（产品要求「颜色再深一点」，承接同日四次定案的「列表行按卡片色派生」）：六档一起加深，保持表格内部相对关系，卡片实色不动。`design-system.css` 的列表行变量：紧急 `#ce342b` / `#fdf2f1` / `#fbe4e2` / `#f2d3d0` / `#a52a22` → `#be3028` / `#fae0dd` / `#f5cfcb` / `#eec4bf` / `#8f1f18`（7.06:1）；「高」`#fdc106` / `#fff8de` / `#fdecbf` / `#f4e2a6` / `#6b4c00` → `#e6b005` / `#fff3c9` / `#fbe7a6` / `#efd88c` / `#5f4500`（8.09:1）；普通 `#a0adb9` / `#f6f8fa` / `#ebeff3` / `#dfe5ea` / `#5f6d7a` → `#8b99a7` / `#eef2f6` / `#e1e8ee` / `#d2dbe3` / `#51606e`；已完成 `#1d5b62` / `#e1f2f2` / `#d6edee` / `#bcdddf` → `#17535a` / `#d3ebec` / `#c6e3e4` / `#a6d1d4`；已取消 `#aab7c4` / `#f5f7f9` / `#ebeff3` / `#e3e8ed` / `#75828f` → `#96a4b2` / `#e9edf1` / `#dbe2e9` / `#ccd5de` / `#5b6875`（压得更冷一档，避免与「普通」行分不出来，对比度 3.66:1 → 5.03:1）；遗留问题 `#8a2b06` / `#fbeee9` / `#f7e3da` / `#f0d5c9` → `#7c2705` / `#f7dfd3` / `#f2cdbd` / `#e8bda8`。看板列表行 `.tb-row--*` 六块同值；看板优先级标签改为跟随该档行文字色（`.tb-prio--red` `#c02b23` → `#8f1f18`、`.tb-prio--amber` `#8a6e00` → `#5f4500`、`.tb-prio--gray` `#8b99a7` → `#5f6d7a`），因为底色加深后灰标签对底只剩 2.59:1。无头 Chromium 注入真实样式表实测计算值：紧急行底 `rgb(250, 224, 221)` / 色条 `rgb(190, 48, 40)` / 标题 `rgb(143, 31, 24)`，「高」行底 `rgb(255, 243, 201)` / 色条 `rgb(230, 176, 5)` / 标题 `rgb(95, 69, 0)`，普通 `rgb(238, 242, 246)` / `rgb(139, 153, 167)` / `rgb(81, 96, 110)`，已完成 `rgb(211, 235, 236)` / `rgb(23, 83, 90)`，已取消 `rgb(233, 237, 241)` / `rgb(150, 164, 178)`，遗留问题 `rgb(247, 223, 211)` / `rgb(124, 39, 5)`。样图未重出。 |
| 2026-09-23 | 遗留问题来源退出紧急桶（用户反馈「现在这个任务排序不是很好，遗留问题只需要比同优先级的高就行了」，截图为 `A-6 上线门禁` 高优先级卡被两张「遗留问题」普通优先级卡压住）：服务端 `task-list-order.ts` 的紧急桶收窄为「标记紧急(0) → 已逾期(1) → 今/明日截止(2) → 其余(3)」，新增紧随优先级之后的独立一级「遗留问题来源(0) → 其余(1)」（仅未完成参与，非未完成分组恒 1）；`TASK_LIST_SORT_KEY_VERSION` 由 4 升到 5（载荷 7 段 → 8 段，旧游标按无效游标拒绝）。前端 `TaskCenterPageView.tsx` 的 `urgencyBucketOf` 去掉遗留问题分支、新增 `leftoverRankOf`，`gridEntries` 与组卡 rank 同步插入一级。反向验证：`aggregate-read-ports.integration.test.ts` 新增「高优先级无遗留」与「同优先级无遗留」两条对照夹具，断言高优先级任务仍排在遗留问题来源之前。 |
| 2026-09-23 | 列表「普通」行改蓝边白底（产品要求「普通的任务在列表模式下改为蓝色边边，白色底」，指任务中心 `/tasks?view=list` 的列表视图；附图为四行任务截图，其中「普通」行当时还是 `#eef2f6` 近白中性底 + 灰条 `#8b99a7`）：列表行 / 表格行的 `.tone-prio-normal` 由 `#8b99a7` / `#eef2f6` / `#e1e8ee` / `#d2dbe3` / `#51606e` 换成 `#1467d8` / `#ffffff` / `#f2f7fe` / `#dbe6f5` / `#51606e`——白底靠左侧 3px 蓝边与单元格分隔线（`#edf1f6`）分界，行内文字仍取中性深色 `#51606e`（对白底 6.6:1）。卡片侧刻意不跟随：卡片实色块 `:is(.calm-task-card, .tb-card).tone-prio-normal` 新增 `--task-c: #8b99a7`，把白底卡自己的中性灰竖条冻结住，避免卡片也被改成蓝色；看板列表行 `.tb-row--tone-gray` 同值同步（类名 gray 只来自 `task-tone` 的色调映射）。其余五档不变。无头 Chromium 1340 宽注入真实样式表实测计算值：普通行底 `rgb(255, 255, 255)` / 色条 `rgb(20, 103, 216)` / 标题 `rgb(81, 96, 110)`，白底卡的 `::before` 竖条仍是 `rgb(139, 153, 167)`；紧急 `rgb(250, 224, 221)` / `rgb(190, 48, 40)` / `rgb(143, 31, 24)`、高 `rgb(255, 243, 201)` / `rgb(230, 176, 5)` / `rgb(95, 69, 0)`、已完成 `rgb(211, 235, 236)` / `rgb(23, 83, 90)`、已取消 `rgb(233, 237, 241)` / `rgb(150, 164, 178)`、遗留问题 `rgb(247, 223, 211)` / `rgb(124, 39, 5)` 均未变。样图未重出。 |
| 2026-09-23 | 「已完成」列表行改用卡片青碧（产品要求「我想 p1 用 p2 的颜色」：p1 是任务中心 `/tasks?status=done` 已完成列表行的截图，p2 是卡片青碧的截图）：列表行 / 表格行的 `.tone-prio-done` 由 `#17535a` / `#d3ebec` / `#c6e3e4` / `#a6d1d4` / `#17535a` 改成 `#17535a` / `#e1f2f2` / `#d6edee` / `#bcdddf` / `#17535a`，与卡片实色同值；看板列表行 `.tb-row--done` 同值同步。卡片侧本来就是这一组（`:is(.calm-task-card, .tb-card).tone-prio-done` 自 2026-09-22 D 青碧定案起就是 `#e1f2f2` / `#bcdddf` / `#d6edee`），改动前后计算值一致。「已完成」这一档因此退出五次定案「列表行比卡片深一档」的规律，其余五档不变。无头 Chromium 1360 宽注入真实样式表实测计算值：已完成行常态底 `rgb(225, 242, 242)` / 色条 `rgb(23, 83, 90)` / 标题 `rgb(23, 83, 90)`，悬停（等 0.15s 过渡结束再读）`rgb(214, 237, 238)`；卡片 `rgb(225, 242, 242)` / 描边 `rgb(188, 221, 223)` 在改动前后同值；紧急 `rgb(250, 224, 221)` / `rgb(190, 48, 40)`、高 `rgb(255, 243, 201)` / `rgb(230, 176, 5)`、普通 `rgb(255, 255, 255)` / `rgb(20, 103, 216)`、已取消 `rgb(233, 237, 241)` / `rgb(150, 164, 178)`、遗留问题 `rgb(247, 223, 211)` / `rgb(124, 39, 5)` 均未变。样图重出为《已完成行用卡片青碧-对照》。 |
| 2026-09-24 | 遗留问题来源改排到已逾期之后（产品要求「这个排序稍微改一下，把遗留问题排到已经逾期后面」）：服务端紧急桶由「遗留问题来源(0) → 标记紧急(1) → 已逾期(2) → 今/明日截止(3) → 其余(4)」改为「标记紧急(0) → 已逾期(1) → 遗留问题来源(2) → 今/明日截止(3) → 其余(4)」——遗留问题来源整体降到已逾期之后一档，「今/明日截止」与「其余」两桶不动；`TASK_LIST_SORT_KEY_VERSION` 由 4 升到 5（旧的 0 由「遗留问题来源」改指「标记紧急」，继续接受旧游标会把遗留问题任务当紧急任务排到最前，故整版拒绝）。前端 `TaskCenterPageView.tsx` 的 `urgencyBucketOf` 与服务端同步重排（组卡与任务卡共用这一处判定），卡片与列表行的取色、徽章、日期文案均不变——本行只改顺序，不改颜色。**该口径已于同日作废**，见下一行。 |
| 2026-09-24 | 排序口径复核（产品要求「为什么这里排序又被改动了，我要求遗留问题只会在同优先级里面高一点，以后不管是别人拉取还是，都要以这个为准」）：撤销上一行的口径，恢复 2026-09-23 的结构——紧急桶只有「标记紧急(0) → 已逾期(1) → 今/明日截止(2) → 其余(3)」，遗留问题来源是优先级之后的独立一级（仅未完成参与，非未完成分组恒 1），只在同优先级任务内提前、不越过更高优先级；`TASK_LIST_SORT_KEY_VERSION` 由 6 升到 7（载荷回到 8 段，v5 / v6 旧游标整版拒绝）。前端 `TaskCenterPageView.tsx` 的 `urgencyBucketOf` 回到四档、`leftoverRankOf` 保留为独立一级，`gridEntries` 与组卡 rank 同步回到六级；卡片与列表行的取色、徽章、日期文案仍不变。反向验证：`aggregate-read-ports.integration.test.ts` 的顺序矩阵含「高优先级无遗留 vs 普通优先级有遗留」对照（回归防线见 `AGENTS.md` 同名小节）。 |
