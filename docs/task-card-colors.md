...
- `apps/api/src/modules/tasks/task-list-order.ts`：统一排序键的紧急桶 / 完成时间倒序与游标键版本（`TASK_LIST_SORT_KEY_VERSION`，2026-09-22 由 1 升到 2、同日再升到 3：新增「完成时间倒序」一级，已完成分组按 `completed_at` 从晚到早，未完成 / 已取消用 `'-infinity'` 归一后不参与比较；游标载荷由 6 段变 7 段；2026-09-23 随「低」档位下线由 3 升到 4：优先级序号由 0/1/2/3 收窄为 0/1/2，旧游标整版拒绝）。
