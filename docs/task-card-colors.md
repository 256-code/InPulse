...
- 排序：服务端 `apps/api/src/modules/tasks/task-list-order.ts` 的紧急桶（仅未完成参与，命中第一个）把未完成任务排成「标记紧急(0) → 已逾期(1) → 遗留问题来源(2) → 今/明日截止(3) → 其余(4)」——已逾期落在「标记紧急」之后一档，仍排在普通任务之前；遗留问题来源自 2026-09-24（产品要求「把遗留问题排到已经逾期后面」）由第 0 桶降到已逾期之后一档。游标键版本随两次重排由 1 升到 2、2026-09-24 由 4 升到 5，旧游标按无效游标拒绝。
