...
- 未完成组的排序（2026-09-22 产品要求「当组合任务为未完成状态（未开始或进行中）排序按照合并任务以内部未完成任务中的最高优先级」，同日追加「（它们）同样是一个优先级的，按照截止日期从近到远排序」，2026-09-23 追加「遗留问题只需要比同优先级的高就行了」）：组卡不再单独排一遍再追加到任务卡后面，而是与任务卡共用 `gridEntries` 这一把尺子——状态分组（未完成 0 / 已完成 1 / 已取消 2）→ 紧急桶（标记紧急 0 / 已逾期 1 / 今-明日截止 2 / 其余 3）→ 优先级档位（组取未完成分支最高一档）→ 遗留问题来源（仅未完成；组卡无此事实，恒为 1）→ 截止时间近到远（组取未完成分支中最早的一条，未设截止排最后）。都取自服务端 `apps/api/src/modules/tasks/task-list-order.ts` 的排序键，组卡的紧急桶 / 优先级 / 截止三项都从「未完成分支」派生；卡片视图与列表视图同一顺序，同键保持稳定次序（任务在前、组保持 R-7 顺序）。`visibleGroups` 现在只做「未完成 / 已完成」归属筛选。
...
- 排序：服务端 `apps/api/src/modules/tasks/task-list-order.ts` 的紧急桶（仅未完成参与，命中第一个）把未完成任务排成「标记紧急(0) → 已逾期(1) → 今/明日截止(2) → 其余(3)」——已逾期落在「标记紧急」之后一档，仍排在普通任务之前；遗留问题来源自 2026-09-23 起退出紧急桶，改为优先级之后的独立一级（只在同优先级内提前）。游标键版本随之由 1 升到 5（2：逾期退到紧急之后；3：新增「完成时间倒序」；4：删「低」档位；5：遗留问题独立成级），旧游标按无效游标拒绝。
...
- `apps/api/src/modules/tasks/task-list-order.ts`：统一排序键的紧急桶 / 完成时间倒序 / 遗留问题来源与游标键版本（`TASK_LIST_SORT_KEY_VERSION`，2026-09-22 由 1 升到 2、同日再升到 3：新增「完成时间倒序」一级，已完成分组按 `completed_at` 从晚到早，未完成 / 已取消用 `'-infinity'` 归一后不参与比较；游标载荷由 6 段变 7 段；2026-09-23 随「低」档位下线由 3 升到 4：优先级序号由 0/1/2/3 收窄为 0/1/2；同日再升到 5：遗留问题来源退出紧急桶、改为优先级之后的独立一级，紧急桶收窄为 0..3、载荷由 7 段变 8 段；各次都整版拒绝旧游标）。
