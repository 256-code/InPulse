...
- 未完成组的排序（2026-09-22 产品要求「当组合任务为未完成状态（未开始或进行中）排序按照合并任务以内部未完成任务中的最高优先级」，同日追加「（它们）同样是一个优先级的，按照截止日期从近到远排序」）：组卡不再单独排一遍再追加到任务卡后面，而是与任务卡共用 `gridEntries` 这一把尺子——状态分组（未完成 0 / 已完成 1 / 已取消 2）→ 紧急桶（标记紧急 0 / 已逾期 1 / 遗留问题来源 2 / 今-明日截止 3 / 其余 4；2026-09-24 起）→ 优先级档位（组取未完成分支最高一档）→ 截止时间近到远（组取未完成分支中最早的一条，未设截止排最后）。前三项都取自服务端 `apps/api/src/modules/tasks/task-list-order.ts` 的排序键，组卡的三项都从「未完成分支」派生；卡片视图与列表视图同一顺序，同键保持稳定次序（任务在前、组保持 R-7 顺序）。`visibleGroups` 现在只做「未完成 / 已完成」归属筛选。
...
- 排序：服务端 `apps/api/src/modules/tasks/task-list-order.ts` 的紧急桶（仅未完成参与，命中第一个）把未完成任务排成「标记紧急(0) → 已逾期(1) → 遗留问题来源(2) → 今/明日截止(3) → 其余(4)」——已逾期落在「标记紧急」之后一档，仍排在普通任务之前；遗留问题来源自 2026-09-24（产品要求「把遗留问题排到已经逾期后面」）由第 0 桶降到已逾期之后一档。游标键版本随两次重排由 1 升到 2；2026-09-23 并行开发线曾由 4 升到 5（遗留问题来源独立成级、载荷 8 段），2026-09-24 两条线合并后统一回本口径并由 5 升到 6（载荷 7 段），旧游标一律按无效游标拒绝。
...
- `apps/api/src/modules/tasks/task-list-order.ts`：统一排序键的紧急桶 / 完成时间倒序与游标键版本（`TASK_LIST_SORT_KEY_VERSION`，2026-09-22 由 1 升到 2、同日再升到 3：新增「完成时间倒序」一级，已完成分组按 `completed_at` 从晚到早，未完成 / 已取消用 `'-infinity'` 归一后不参与比较；游标载荷由 6 段变 7 段；2026-09-23 随「低」档位下线由 3 升到 4：优先级序号由 0/1/2/3 收窄为 0/1/2，旧游标整版拒绝；2026-09-24 由 4 升到 5：紧急桶重排为「标记紧急(0) → 已逾期(1) → 遗留问题来源(2) → 今/明日截止(3) → 其余(4)」，旧的 0 由「遗留问题来源」改指「标记紧急」）。
