...
正式记录提供“转为新任务”，保留真实 MODULE/FEATURE 范围，预览遗留全文及哪些影响继承/排除；用户确认负责人，可改标题/优先级，并设置可选截止时间。任务说明由服务端拼入来源记录编号、正式版本、稳定遗留 ID 与本次精确遗留原文，不能由客户端覆写。成功可跳任务，任务详情可返回来源正式记录；CONVERTED 永远提供原任务链接。
...
未跑全仓lint/typecheck/build/test、无关审计或本批GitHub CI。未启动F21。父协调要求剩余Edge路径完成后冻结，未扩大测试。独占隔离PostgreSQL已核对目录后正常停止，端口55424释放并保留数据；最终文档SHA随交付消息提供。父审核指出最初遗漏功能§17.2的可选截止时间，已在本次交审前补齐，不再推迟普通编辑。输入沿用既有任务表单的datetime-local与本地时间→UTC ISO语义；清空传null，非法日期安全提示并阻止提交。截止时间在失败/409刷新确认期间保留，参与请求签名：同时间同语义失败复用Key，改时间或清空换Key。

## 截止时间审核补齐（2026-09-10 16:13）

父协调指出原候选将截止时间推迟到普通任务编辑，不符合功能§17.2。本次补齐可选datetime-local输入，沿用既有任务表单本地时间转UTC ISO的语义，非法格式/无效日期先安全校验，清空传null。时间在网络失败和409刷新确认中保留，并参与body签名：相同请求复用Key，改变时间或清空产生新Key。代码增量4636f0e8340be58c453bd66614fb0a67c167652c，未改API/数据库。

定向Web ConvertLeftoverTask.test.tsx 4/4（3.19秒），覆盖选定时间UTC值、失败保留/同请求同Key、409确认保留、改时间换Key、清空null及非法日期不抛异常。现有Edge F20 FEATURE单例增加填写截止时间并在普通任务编辑页校验准确回显，现有CSP build+preview下1/1（16.1秒）。首次Web因弹窗尚未可见时立即断言失败，改为等待原可见性断言；首次Edge时间回显断言通过后因“取消”实际可访问名为“取 消”超时，改用既有中文按钮空白匹配并从真实任务URL重开详情，随后通过，未降低日期断言。局部格式检查发现E2E排版已修正。

父协调另独立复跑原4真库文件82/82（9.71秒）与4契约文件41/41，未发现新的服务端阻断；本次前端增量未重复这两套。隔离库在协调明确释放后仅为单条Edge启动，结束已核对目录并正常停止，55424无监听、数据保留。最终候选仍未推送/PR/CI，等待增量审核。
