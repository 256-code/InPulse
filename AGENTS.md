...
- 阶段 0 前端 MFA 与管理员重认证交互已本地落库：登录页支持 TOTP 注册、验证与恢复码并在成功后轮换 CSRF Token；管理员账户菜单提供“管理员安全验证”弹窗，输入管理员密码和当前 TOTP 完成 5 分钟双因子重认证；所有前端调用经生成客户端并统一映射 401/403/409/422/429；新增前端单测（Web 25 文件 63 例）与 Playwright MFA 真实 UI 用例，E2E 总 8/8、API 单测 47 文件 224 例、真实 PostgreSQL 集成 22 文件 89 例均本地通过；lint、typecheck、build、格式、依赖边界、Secret 与文档检查通过；GitHub Actions 尚未执行。
