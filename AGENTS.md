...
- 阶段 0 F-31 Playwright 浏览器测试基座已本地落库：新增 pps/e2e（Playwright webServer 启动 API/Vite、global setup/teardown、真实 UI 登录辅助、API 启动探针/匿名保护页/全局搜索/站内通知/API Session 5 个用例）、根级 pnpm test:e2e、Vite 代理与 CI / workspace 步骤；CI 步骤按 §12.4 位于 pnpm build 与 pnpm check:deps 之间；本地 5/5 通过，配套非数据库门禁与真实 PostgreSQL 集成均通过，完整业务关键路径仍未覆盖，Playwright 新增强需非作者人工评审。
