...
- 阶段 0 F-27/F-28 项目动态与通知状态 Playwright E2E 已本地落库：新增 `activity.spec.ts`，覆盖创建项目后进入专属动态页并看到 `project.create` 条目；`notifications.spec.ts` 扩为已读 ↔ 未读切换、全部已读与铃铛未读计数联动；抽取 `createProjectViaUi` 供动态/通知场景复用。本分支已 rebase 到 `origin/main` `8b895e1`；本地完整 `pnpm test:e2e` 14/14（含搜索边界），`pnpm build`、`apps/e2e` typecheck、`pnpm format:check`、`pnpm check:docs` 与 `git diff --check` 通过；PR #67 首次 CI 已通过（workspace 9m58s，docs 通过），rebase 后 GitHub Actions 已通过（workspace 11m7s，docs 通过，run 34334259877/34334259709）。
