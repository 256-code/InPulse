...
- 阶段 0 仍待建立的入口：应用层 E2E（Playwright 关键路径）、生产容器镜像构建、Compose 渲染与 exact-tag/digest 格式校验、镜像扫描，以及生产加密备份恢复。统一的根级安装、lint、格式、类型检查、单元测试、集成测试、生成物漂移检查和构建入口已落库；其余入口只有实际脚本落库后才能把命令写成可执行说明。
