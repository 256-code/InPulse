...
describe("RequireAuth & RequireAdmin", () => {
  ...
  it("renders protected content when user is authenticated", () => {
    render(
      <AuthStateProvider value={{ status: "authenticated", user: { id: 1, username: "test", isSystemAdmin: false } }}>
        <RequireAuth>
          <ProtectedComponent />
        </RequireAuth>
      </AuthStateProvider>
  ...
  it("blocks non-admin user from admin area", () => {
    render(
      <AuthStateProvider value={{ status: "authenticated", user: { id: 2, username: "user", isSystemAdmin: false } }}>
        <RequireAdmin>
          <ProtectedComponent />
        </RequireAdmin>
      </AuthStateProvider>
  ...
  it("allows admin user to access admin area", () => {
    render(
      <AuthStateProvider value={{ status: "authenticated", user: { id: 3, username: "admin", isSystemAdmin: true } }}>
        <RequireAdmin>
          <ProtectedComponent />
        </RequireAdmin>
      </AuthStateProvider>
