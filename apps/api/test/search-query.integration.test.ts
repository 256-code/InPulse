...
} from "../src/modules/search/search-projection.reader";
import {
  SearchQueryService,
...
class FixtureScopedProjectAccessQueryPort implements ProjectAccessQueryPort {
  ...
  async getAuthorizedSearchScope(
    ...
    const [user] = await this.#sql<Array<{
    ...
    if (user.is_admin) {
      return {
        actorUserId,
        projectIds: [...this.#fixtureProjectIds].sort((left, right) => left - right),
        isSystemAdmin: true
      };
    }

    const memberships = await this.#sql<Array<{
...
describe("SearchQueryService with real PostgreSQL", () => {
  ...
  test("active member only searches their own project", async () => {
    ...
    expect(result.items.every((item) => item.projectId === projectIds.get("p1"))).toBe(true);
    expect(
      result.items.some((item) => item.entityId === adminOnlyEntityId)
    ).toBe(false);
    expect(
      result.items.some((item) => item.entityId === hiddenEntityId)
    ).toBe(false);
  });
  ...
  test("golden query recall at top-20 remains at least 90%", async () => {
    ...
    for (const spec of normalCases) {
      ...
      const expectedProjectId = spec.expectedProjectKey === null
        ? undefined
        : projectIds.get(spec.expectedProjectKey);
      const found = expectedEntityId !== undefined &&
