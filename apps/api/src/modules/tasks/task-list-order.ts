 ...
 *      但仍在未完成的普通任务之前。遗留问题来源 F 不在这一级。
 *   4. 优先级：紧急(0) → 高(1) → 普通(2)
 *   5. 遗留问题来源（仅未完成参与）：遗留问题来源(0) → 其余(1)
 *   6. 截止时间：due_at 升序，NULL 最后（用 'infinity' 归一，便于 keyset 比较）
 *   7. 任务 ID 升序：唯一兜底，保证刷新前后顺序稳定
 *
 * 第 5 级是 2026-09-24 的产品定案（原文「遗留问题只会在同优先级里面高一点……
 * 以后不管是别人拉取还是，都要以这个为准」）：遗留问题来源只用于同优先级内部
 * 提前，绝不越过任何更高优先级的任务。它曾经被并进紧急桶（第 2 桶），会让普通
 * 优先级的遗留问题来源任务压住高优先级任务，属于已撤销口径，不得再改回去。
 *
...
export interface TaskListSortKey {
  ...
  /** 遗留问题来源（仅未完成参与）：命中 app.leftover_task_links 为 0，否则 1。 */
 ...
 * 2026-09-23 由 4 升到 5（并行开发线）：遗留问题来源从紧急桶最高档移到优先级之后的独立一级，
 * 紧急桶序号由 0..4 收窄为 0..3、载荷由 7 段变 8 段，旧游标继续使用会跳页 / 漏项。
 * 2026-09-24 由 5 升到 6：两条并行开发线合并后曾采用「遗留问题来源回到紧急桶、排在已逾期之后」
 * 的口径（紧急桶 标记紧急(0) → 已逾期(1) → 遗留问题来源(2) → 今/明日截止(3) → 其余(4)，载荷 7 段）。
 * 2026-09-24 由 6 升到 7（产品定案，撤销上一行）：遗留问题来源退出紧急桶、回到第 5 级独立比较，
 * 紧急桶收窄为 0..3、载荷回到 8 段；v6（7 段）与更早的 5/4/3/2 游标一律按无效游标拒绝。
 * （更早三次：2 → 3 新增「完成时间倒序」一级；1 → 2 紧急桶重排，已逾期从 0 移到 2、
 * 遗留问题来源升到 0、标记紧急升到 1。）
 */
export const TASK_LIST_SORT_KEY_VERSION = 7;
...
export function taskListSortExpressions(sql: ISql): TaskListSortExpressions {
  ...
  const urgency = sql`CASE
           ...
           END`;
  /**
   * 遗留问题来源：独立一级，落在优先级之后，因此只让同优先级的任务提前、
   * 不越过更高优先级（2026-09-24 产品定案）。非未完成分组恒 1，使这一级在
