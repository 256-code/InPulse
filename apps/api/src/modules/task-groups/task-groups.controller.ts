...
/** F-23 合并与 F-24 解除合并入口：无路径/查询参数，归属由请求体任务 ID 在服务端解析。 */
...
export class TaskGroupsController {
  constructor(
    ...
    @Inject(TaskGroupUnmergeHttpService)
    private readonly unmergeService: TaskGroupUnmergeHttpService,
  ) {}
  ...
  @Post("unmerge")
  @Operation("unmergeTaskGroup")
  async unmergeTaskGroup(
    @Req() request: TaskGroupHttpRequest,
    @Res({ passthrough: true }) response: Response,
    @ContractBody("unmergeTaskGroup") body: unknown,
    @ContractHeaders("unmergeTaskGroup") _headers: unknown,
  ) {
    const result = await this.unmergeService.handle({
      headers: request.headers,
      params: {},
      query: request.query,
      body,
