...
export class RecordLinkQueryPort implements LinkTargetQueryPort {
  async find(
    ...
    const [row] = await tx.sql<
      LinkTarget[]
    >`SELECT id,project_id AS "projectId",module_id AS "moduleId",feature_id AS "featureId",status AS status,row_version AS "rowVersion" FROM app.change_records WHERE id=${id} ${lock ? tx.sql`FOR UPDATE` : tx.sql``}`;
    return row;
  }
}
@Injectable()
