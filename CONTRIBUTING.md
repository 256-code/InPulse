...
A 已确认 envelope 与不透明游标方向并关闭 C-006；搜索页面与真实 HTTP API
集成已落库，完整 Playwright 关键路径 E2E 与生产加密备份恢复仍未完成。
...
- 生产加密备份恢复（F-10.2）演练、真实镜像 Tag/digest 签名发布清单（备份服务本体、升级/回滚规程、首次建库 `compose.init` 一次性覆盖与灾难恢复离线 Runbook 均已交付）；
- 镜像漏洞扫描。
