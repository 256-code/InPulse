...
| 下一步 | A 已正式确认 `{ items, nextCursor, hasMore }` 与不透明游标方向（2026-09-08），C-006 关闭；搜索页面最小纵切片、F-27/F-28 读写与前端纵切片已本地完成，接下来把项目创建、任务完成、记录作废/恢复等业务 Workflow 接入活动/通知写端口；补齐 Playwright 完整关键路径 E2E、生产 Dockerfile、真实镜像 digest、镜像扫描与加密备份恢复；Playwright 测试基座、Compose 渲染与 ref 预检已作为阶段 0 门禁落库；真实 HTTP API 集成与前端页面将在 PR 上由 CI 复核；CI 在真实 PR 上稳定后再把 §12.4 门禁设为 required checks |
