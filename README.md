...
| 下一步 | A 已正式确认 `{ items, nextCursor, hasMore }` 与不透明游标方向（2026-09-08），C-006 关闭；搜索页面最小纵切片、F-27/F-28 读写与前端纵切片已本地完成，接下来把项目创建、任务完成、记录作废/恢复等业务 Workflow 接入活动/通知写端口；补齐 Playwright 完整关键路径 E2E、生产 Dockerfile/Compose 与镜像扫描门禁、加密备份恢复；真实 HTTP API 集成与前端页面将在 PR 上由 CI 复核；CI 在真实 PR 上稳定后再把 §12.4 门禁设为 required checks |
...
| 尚未提供 | 业务领域模块与 API、Playwright 完整关键路径 E2E、生产容器镜像与 Compose 部署产物、镜像扫描与备份恢复门禁 |

下列根级命令已真实可运行，并与 GitHub Actions 的 `CI / workspace` job 按[技术设计 §12.4](./技术设计v1.2.2.md#124-ci-门禁)顺序执行同一组命令；§12.4 中 Playwright 完整关键路径、生产容器镜像构建、Compose 渲染与 digest 校验、镜像扫描尚未落库，补齐前请勿假设这些检查已执行。
...
Playwright 基座位于 `apps/e2e`，通过 `webServer` 启动已构建的 API 与 Vite dev
server，并创建唯一 E2E 用户、项目成员和搜索投影。当前覆盖 API 健康探针、匿名
保护页、真实 UI 登录、全局搜索和站内通知冒烟，不等同于完整业务关键路径；复现说明见
[E2E 说明](./apps/e2e/README.md)。
