...
| 尚未提供 | 业务领域模块与 API、其余 Playwright 完整关键路径 E2E、生产容器镜像、真实镜像 digest、镜像扫描与备份恢复门禁；Playwright 测试基座、Compose 稳态拓扑与镜像 ref 预检已提供 |
