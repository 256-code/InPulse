...
| 下一步 | A 已正式确认 `{ items, nextCursor, hasMore }` 与不透明游标方向（2026-09-08），C-006 关闭；接下来接入搜索页面、继续认证与业务领域模块；补齐真实 HTTP API 集成、Playwright E2E、生产 Dockerfile/Compose 与镜像扫描门禁、加密备份恢复；CI 在真实 PR 上稳定后再把 §12.4 门禁设为 required checks |
| 已提供 | pnpm workspace、严格 TypeScript、格式检查、数据库类型检查/迁移/集成测试、应用 typecheck/lint/build、前端单元测试（`pnpm test:web`）与 dependency-cruiser 边界检查、契约生成与漂移检查、权限矩阵检查、依赖边界与 Secret 扫描、依赖漏洞审计、搜索服务真实 PostgreSQL 集成测试、搜索 API 稳定正式契约（C-006 已确认）、PGroonga 与原 pg_trgm 搜索 PoC 工具链，以及文档检查 |
| 尚未提供 | 真实 HTTP API 集成与搜索页面、业务领域模块与 API、Playwright E2E、生产容器镜像与 Compose 部署产物、镜像扫描与备份恢复门禁 |
