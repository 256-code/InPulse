...
| 下一步 | 在契约平台（[ADR-027](./docs/adr/ADR-027.md) 已 `Accepted`）基础上接入搜索 API/Controller、生产 `ProjectAccessQueryPort` 适配器与前端页面；随后继续认证与业务领域模块；补齐 Playwright E2E、生产 Dockerfile/Compose 与镜像扫描门禁、加密备份恢复；CI 在真实 PR 上稳定后再把 §12.4 门禁设为 required checks |
