...
export const routeRegistry = [
  ...
  },
  {
    ...
    summary:
      "系统管理员读取原始审计链；要求当前有效的完整管理员 Session（is_admin），不要求 CSRF；AuditQueryService 使用独立 audit_reader 只读连接查询，返回前必须由 app_runtime 通过受限追加函数向 SYSTEM 链写入 AUDIT_LOG_READ（查询条件与返回条数，不记录返回正文），留痕失败则整体失败；不传 projectId 读 SYSTEM 链，传 projectId 读 PROJECT:<id> 链。",
    request: {
      path: "none",
  ...
  },
  {
    ...
    summary:
      "读取当前认证用户可见项目；系统管理员返回全部项目，普通用户只返回存在 ACTIVE 成员关系的项目，包含归档历史；按生命周期档位（正常、未开始、已归档）分组，组内再按项目 ID 升序；每个条目附带当前用户在本项目的成员角色与待审归档申请摘要（ADR-034），供列表页决定申请与审核入口；SQL 前使用服务端 AuthorizedProjectScope，不接受客户端传入授权范围。",
    request: {
      path: "none",
      query: "none",
      headers: "none",
