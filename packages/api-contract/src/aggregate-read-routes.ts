...
export const aggregateReadRoutes: readonly RouteDefinition[] = [
  ...
  },
  {
    ...
    summary:
      "R-7 任务聚合组列表（F-25）：跨项目按 AuthorizedProjectScope 汇总当前用户作为活跃分支任务负责人的聚合组（与自己无关的组不返回，组关闭后成员全部解除因此不再返回）；分支主任务在前、来源任务按 joinedAt 升序，携带优先级、工作状态、模块/功能名称、截止时间与 PUBLISHED 迭代记录数；已解除成员不进入摘要；按 groupId DESC 游标分页，服务端返回原始状态枚举，组的派生展示值（组优先级、归属、截止、迭代合计）由客户端按事实计算。",
    request: {
      path: "none",
